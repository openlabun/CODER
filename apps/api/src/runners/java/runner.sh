...
SRC=$(echo "$payload" | python3 -c "import sys,json; print(json.load(sys.stdin).get('source_file','/code/Main.java'))")
...
# === Compilar el código ===
if ! javac -d /runner "$SRC" 2> /runner/compile.err; then
  python3 - <<'PY'
import json,sys
print(json.dumps({
...
for inpath in $(ls /tests/*.in 2>/dev/null | sort); do
  ...
  timeout --signal=KILL "${TIME_LIMIT_SEC}s" java -cp /runner Main < "$inpath" > /runner/exec.out 2> /runner/exec.err || true
  end=$(date +%s%3N)
  elapsed=$((end - start))
  total=$((total + elapsed))
