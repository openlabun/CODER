...
func MapCreateCourseInputToCourseEntity(professorID string, input dtos.CreateCourseInput) (*Entities.Course, error) {
	...
	period := Entities.Period{
		Year: input.Year,
		Semester: Entities.AcademicPeriod(input.Semester),
	}

	visibility := Entities.CourseVisibility(input.Visibility)
	visual_identity := Entities.CourseColour(input.VisualIdentity)
...
func MapUpdateCourseInputToCourseEntity(originalCourse *Entities.Course, input dtos.UpdateCourseInput) (*Entities.Course, error) {
	...
	if input.Visibility != nil {
		originalCourse.Visibility = Entities.CourseVisibility(*input.Visibility)
	}
	if input.VisualIdentity != nil {
		originalCourse.VisualIdentity = Entities.CourseColour(*input.VisualIdentity)
	}
	if input.Code != nil {
