...
func (uc *GetExamDetailsUseCase) Execute(ctx context.Context, input dtos.GetExamDetailsInput) (*Entities.Exam, error) {
	...
	if role == user_constants.UserRoleProfessor && exam.ProfessorID != user.ID {
	...
	if role == user_constants.UserRoleStudent && exam.Visibility != constants.VisibilityPublic && exam.Visibility != constants.VisibilityCourse {
		return nil, fmt.Errorf("user does not have permissions to view exam details")
	}

	// [STEP 5] If user is student, get its courses and verify that at least one of them is the course of the exam
	if role == user_constants.UserRoleStudent && exam.Visibility == constants.VisibilityCourse && exam.CourseID != nil {
