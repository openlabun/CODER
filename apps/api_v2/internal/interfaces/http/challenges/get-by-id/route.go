...
func Handler(appContainer *container.Application) fiber.Handler {
	return func(c *fiber.Ctx) error {
		result, err := appContainer.ChallengeModule.GetChallengeDetails.Execute(
			shared.BuildRequestContext(c),
			ToInput(MapPath(c.Params("id"))),
		)
		if err != nil {
			return shared.HandleError(c, err)
		}
		return c.Status(fiber.StatusOK).JSON(common.MapChallengeToResponse(result))
	}
}
