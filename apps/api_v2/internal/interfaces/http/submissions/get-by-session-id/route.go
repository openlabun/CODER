...
func Handler(appContainer *container.Application) fiber.Handler {
	return func(c *fiber.Ctx) error {
		path := MapPath(c.Params("sessionId"))
		query := MapQuery(c.Query("status"), c.Query("testId"), c.Query("challengeId"))
		result, err := appContainer.SubmissionUseCases.GetSessionSubmissions.Execute(shared.BuildRequestContext(c), ToInput(path, query))
		if err != nil {
