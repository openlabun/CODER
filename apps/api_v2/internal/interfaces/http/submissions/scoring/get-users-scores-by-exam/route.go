...
func Handler(appContainer *container.Application) fiber.Handler {
	return func(c *fiber.Ctx) error {
		path := MapPath(c.Params("examId"))
		query := MapQuery(c.Query("course_id"))
		input := ToInput(path, query)
		result, err := appContainer.ScoringModule.GetUsersScoresByExam.Execute(shared.BuildRequestContext(c), input)
		if err != nil {
